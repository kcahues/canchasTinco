...
class GoogleCalendarApi {
	...
	private function getTimezoneOffset($timezone = 'America/Guatemala'){
	...
	public function UpdateCalendarEvent($access_token, $calendar_id, $event_id, $event_data, $all_day, $event_datetime, $event_timezone) {
		// Construye la URL de la API para actualizar el evento específico
		...
		curl_setopt($ch, CURLOPT_URL, $apiURL);
		curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
		curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'PUT'); // Cambia a PUT
		curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE);
		curl_setopt($ch, CURLOPT_HTTPHEADER, array('Authorization: Bearer ' . $access_token, 'Content-Type: application/json'));
		curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode($curlPost));
		$data = json_decode(curl_exec($ch), true);
...
{
    // Construye la URL de la API para eliminar el evento específico
    $apiURL = self::CALENDAR_EVENT . $calendar_id . '/events/' . $event_id;

    $ch = curl_init();
    curl_setopt($ch, CURLOPT_URL, $apiURL);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
    curl_setopt($ch, CURLOPT_CUSTOMREQUEST, 'DELETE'); // Cambia a DELETE
    curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, FALSE);
    curl_setopt($ch, CURLOPT_HTTPHEADER, array('Authorization: Bearer ' . $access_token));
    ...
    return true; // El evento se ha eliminado con éxito
}
