...
if (isset($_GET["filtro"])) {
    $filtro = "%" . $_GET["filtro"] . "%";
    $consulta = "SELECT * FROM estadoreserva WHERE descripcion LIKE ? ORDER BY idEstadoReserva ASC";
    ...
    $totalRegistrosConsulta = "SELECT COUNT(*) as total FROM estadoreserva WHERE descripcion LIKE ?";
    ...
    $consulta = "SELECT * FROM estadoreserva ORDER BY idEstadoReserva ASC LIMIT $registrosPorPagina OFFSET $offset";
    $resultados = $conexion->query($consulta);

    // Consulta para contar el número total de registros sin filtro
    $totalRegistrosConsulta = "SELECT COUNT(*) as total FROM estadoreserva";
...
<div class="row">
  <div class="col-md-12">
<h1 class="mb-4 text-orange ">Estado Reserva</h1>
  </div>
</div>
...
<tbody>
    ...
    <tr>
        <td><?php echo $fila["idEstadoReserva"]; ?></td>
        <td><?php echo $fila["descripcion"]; ?></td>
        <td>
        <button class="btn btn-info btn-modificar" data-id="<?php echo $fila["idEstadoReserva"]; ?>" data-descripcion="<?php echo $fila["descripcion"]; ?>">Modificar</button>
        <button class="btn btn-danger btn-eliminar" data-id="<?php echo $fila["idEstadoReserva"]; ?>">Eliminar</button>
            <!-- <button class="btn btn-info btn-modificar" data-id="<?php echo $fila["idEstadoReserva"]; ?>" data-descripcion="<?php echo $fila["descripcion"]; ?>">Modificar</button>
...
<div class="row">
    <div class="col-md-12">
        <h2 class="mb-4 text-orange">Crear Nuevo Estado de reserva</h2>
        <form id="form-crear-rol">
            <div class="form-group">
                <label for="descripcion">Descripción del estado de reserva</label>
                <input type="text" class="form-control" id="descripcion" name="descripcion" required>
            </div>
            <button type="submit" class="btn btn-primary">Crear estado de reserva</button>
        </form>
    </div>
</div>
...
$(document).on('click', '.btn-modificar', function () {
    const idEstadoReserva = $(this).data('id');
    const descripcionActual = $(this).data('descripcion');
    
    Swal.fire({
        title: 'Modificar Tipo anticipo',
        html: `<input id="swal-input1" class="swal2-input" value="${idEstadoReserva}" readonly>
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                type: 'POST',
                url: '/admin/mantenimientos/modificar_estador.php',
                data: { idEstadoReserva: idEstadoReserva, nuevaDescripcion: nuevaDescripcion },
                success: function (response) {
                    if (response === 'success') {
                        Swal.fire('Éxito', 'El estado de reserva se ha modificado correctamente', 'success').then(() => {
                            window.location.href = 'estado_reserva.php';
                        });
                    } else {
                        Swal.fire('Error', 'Hubo un problema al modificar el estado de reserva', 'error');
                    }
                },
                error: function () {
...
$(document).on('click', '.btn-eliminar', function () {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                type: 'POST',
                url: '/admin/mantenimientos/eliminar_estador.php', // Cambia la URL a la que corresponda en tu proyecto
                data: { idEstadoReserva: idEstadoReserva },
                success: function (response) {
                    if (response === 'success') {
                        Swal.fire('¡Eliminado!', 'El estado de reserva ha sido eliminado.', 'success').then(() => {
                            // Recargar la página o realizar alguna otra acción después de eliminar
                            window.location.reload();
                        });
                    } else {
                        Swal.fire('Error', 'Hubo un problema al eliminar el estado de reserva', 'error');
                    }
                },
                error: function () {
                    Swal.fire('Error', 'Hubo un error al comunicarse con el servidor', 'error');
    ...
    $(document).ready(function () {
        ...
        $('#form-crear-rol').submit(function (e) {
            ...
            $.ajax({
                type: 'POST',
                url: '/admin/mantenimientos/crear_estador.php', // Reemplaza con la URL correcta en tu proyecto
                data: { descripcion: descripcion },
                success: function (response) {
                    if (response === 'success') {
                        Swal.fire('Éxito', 'El estado de reserva se ha creado correctamente', 'success').then(() => {
                            // Recargar la página o realizar alguna otra acción después de crear el rol
                            window.location.reload();
                        });
                    } else {
                        Swal.fire('Error', 'Hubo un problema al crear el estado de reserva', 'error');
                    }
                },
                error: function () {
                    Swal.fire('Error', 'Hubo un error al comunicarse con el servidor', 'error');
