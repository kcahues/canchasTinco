...
if (isset($_GET["filtro"])) {
    $filtro = "%" . $_GET["filtro"] . "%";
    $consulta = "SELECT * FROM tipoanticipo WHERE descripcion LIKE ? ORDER BY idTipoAnticipo ASC";
    ...
    $totalRegistrosConsulta = "SELECT COUNT(*) as total FROM tipoanticipo WHERE descripcion LIKE ?";
    ...
    $consulta = "SELECT * FROM tipoanticipo ORDER BY idTipoAnticipo ASC LIMIT $registrosPorPagina OFFSET $offset";
    $resultados = $conexion->query($consulta);

    // Consulta para contar el número total de registros sin filtro
    $totalRegistrosConsulta = "SELECT COUNT(*) as total FROM tipoanticipo";
...
<div class="row">
  <div class="col-md-12">
<h1 class="mb-4 text-orange ">Tipos de anticipo</h1>
  </div>
</div>
...
<tbody>
    ...
    <tr>
        <td><?php echo $fila["idTipoAnticipo"]; ?></td>
        <td><?php echo $fila["descripcion"]; ?></td>
        <td>
        <button class="btn btn-info btn-modificar" data-id="<?php echo $fila["idTipoAnticipo"]; ?>" data-descripcion="<?php echo $fila["descripcion"]; ?>">Modificar</button>
        <button class="btn btn-danger btn-eliminar" data-id="<?php echo $fila["idTipoAnticipo"]; ?>">Eliminar</button>
            <!-- <button class="btn btn-info btn-modificar" data-id="<?php echo $fila["idTipoAnticipo"]; ?>" data-descripcion="<?php echo $fila["descripcion"]; ?>">Modificar</button>
...
<div class="row">
    <div class="col-md-12">
        <h2 class="mb-4 text-orange">Crear Nuevo Tipo de anticipo</h2>
        <form id="form-crear-rol">
            <div class="form-group">
                <label for="descripcion">Descripción de la anticipo</label>
                <input type="text" class="form-control" id="descripcion" name="descripcion" required>
            </div>
            <button type="submit" class="btn btn-primary">Crear Tipo de anticipo</button>
        </form>
    </div>
</div>
...
$(document).on('click', '.btn-modificar', function () {
    const idTipoAnticipo = $(this).data('id');
    const descripcionActual = $(this).data('descripcion');
    
    Swal.fire({
        title: 'Modificar Tipo anticipo',
        html: `<input id="swal-input1" class="swal2-input" value="${idTipoAnticipo}" readonly>
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                type: 'POST',
                url: '/admin/mantenimientos/modificar_tipoanticipo.php',
                data: { idTipoAnticipo: idTipoAnticipo, nuevaDescripcion: nuevaDescripcion },
                success: function (response) {
                    if (response === 'success') {
                        Swal.fire('Éxito', 'El tipo de anticipo se ha modificado correctamente', 'success').then(() => {
                            window.location.href = 'tipo_anticipo.php';
                        });
                    } else {
                        Swal.fire('Error', 'Hubo un problema al modificar el tipo de anticipo', 'error');
                    }
                },
                error: function () {
...
$(document).on('click', '.btn-eliminar', function () {
    ...
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            $.ajax({
                type: 'POST',
                url: '/admin/mantenimientos/eliminar_tipoanticipo.php', // Cambia la URL a la que corresponda en tu proyecto
                data: { idTipoAnticipo: idTipoAnticipo },
                success: function (response) {
                    if (response === 'success') {
                        Swal.fire('¡Eliminado!', 'El tipo de anticipo ha sido eliminado.', 'success').then(() => {
                            // Recargar la página o realizar alguna otra acción después de eliminar
                            window.location.reload();
                        });
                    } else {
                        Swal.fire('Error', 'Hubo un problema al eliminar el tipo de anticipo', 'error');
                    }
                },
                error: function () {
                    Swal.fire('Error', 'Hubo un error al comunicarse con el servidor', 'error');
    ...
    $(document).ready(function () {
        ...
        $('#form-crear-rol').submit(function (e) {
            ...
            $.ajax({
                type: 'POST',
                url: '/admin/mantenimientos/crear_tipoanticipo.php', // Reemplaza con la URL correcta en tu proyecto
                data: { descripcion: descripcion },
                success: function (response) {
                    if (response === 'success') {
                        Swal.fire('Éxito', 'El tipo de anticipo se ha creado correctamente', 'success').then(() => {
                            // Recargar la página o realizar alguna otra acción después de crear el rol
                            window.location.reload();
                        });
                    } else {
                        Swal.fire('Error', 'Hubo un problema al crear el tipo de anticipo', 'error');
                    }
                },
                error: function () {
                    Swal.fire('Error', 'Hubo un error al comunicarse con el servidor', 'error');
